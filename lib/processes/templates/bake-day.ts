 ...
 *
 * Anchoring:
 *   - default: T0 = `startedAt` (the moment you start building the levain).
 *   - `options.anchorMode === "ready"`: schedule backwards from the moment
 *     the user wants the bread "ready". `options.readyMeans` decides what
 *     ready means:
 *       - "out_of_oven" (default): loaf comes out of the Dutch oven.
 *         Maps to T0 + ~22h50m (end of bake-uncovered step).
 *       - "fully_cooled": sliceable. Maps to T0 + ~25h30m (cool step end,
 *         ~2h30m on the wire rack).
 */
export const OUT_OF_OVEN_OFFSET_MIN = 22 * 60 + 50; // 22h 50m
export const FULLY_COOLED_OFFSET_MIN = 25 * 60 + 30; // 25h 30m

export type BakeDayOptions = {
  anchorMode?: "start" | "ready";
  targetReadyAt?: string | Date;
  readyMeans?: "out_of_oven" | "fully_cooled";
};
...
export const bakeDayTemplate: Template = {
  ...
  recipeVersion: "v1",
  defaultNickname: (o) => {
    const opts = (o.options ?? {}) as BakeDayOptions;
    if (opts.anchorMode === "ready" && opts.targetReadyAt) {
      const ready = new Date(opts.targetReadyAt);
      return `Loaf ready ${ready.toLocaleString([], { weekday: "short", hour: "2-digit", minute: "2-digit" })}`;
    }
    return `Loaf — ${o.startedAt.toLocaleDateString()}`;
  },
  build: ({ startedAt, options }) => {
    const opts = (options ?? {}) as BakeDayOptions;
    const steps: StepDraft[] = [];
    const reminders: ReminderDraft[] = [];

    const T0 =
      opts.anchorMode === "ready" && opts.targetReadyAt
        ? subMinutes(
            new Date(opts.targetReadyAt),
            opts.readyMeans === "fully_cooled"
              ? FULLY_COOLED_OFFSET_MIN
              : OUT_OF_OVEN_OFFSET_MIN
          )
        : startedAt;
    ...
    const t11 = step({ h: 21 }, "preheat", "Preheat Dutch oven",
      "Dutch oven (lid on) into oven at 260°C. Full hour preheat — cast iron must be saturated.");
    remind(t11, "Preheat the Dutch oven", "Dutch oven (lid on) → 260°C for a full hour.", "preheat");

    // 12. Score + bake covered (T0 + 22h)
    const t12 = step({ h: 22 }, "score-bake-covered", "Flip, score, bake covered",
      "Cold from fridge → onto parchment X → score one ~6\" cut just off-center, ¼\" deep → into the screaming-hot Dutch oven, lid on, 18–20 min at 260°C.");
    remind(t12, "Score and bake covered", "Flip onto parchment, score off-center ¼\" deep, into the Dutch oven covered. 18–20 min at 260°C.", "score-bake-covered", 10);

    // 13. Uncover + finish bake (T0 + 22h 20min)
    const t13 = step({ h: 22, m: 20 }, "bake-uncovered", "Uncover and finish",
      "Lid off (tilt away — steam!). Optionally drop to 230°C. Bake 20–30 min more until deep mahogany; internal temp 98°C.");
    remind(t13, "Uncover the loaf", "Lid off (tilt away). Bake 20–30 min more. Internal temp 98°C.", "bake-uncovered");
